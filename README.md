...
- MAC address генерируется из 96-битного UID чипа (`Application/net_id`): `02:00:` + CRC32(UID).
  Адрес locally-administered (не из купленного OUI) и совпадает с MAC бутлоадера; для серийного
  производства с собственным OUI его нужно будет заменить
